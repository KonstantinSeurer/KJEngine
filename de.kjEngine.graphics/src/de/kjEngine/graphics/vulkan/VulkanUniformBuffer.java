 ...
 *
 */
public class VulkanUniformBuffer extends ShaderBuffer implements VulkanBuffer {

	private long buffer, bufferMemory;
	private ByteBuffer data;
	private BufferAccessor accessor;

	public VulkanUniformBuffer(BufferSource source, List<StructSource> definedStructs, int flags) {
		super(source, definedStructs, flags);

		accessor = new Std140ShaderBufferAccessor(source.asStruct(), definedStructs);

		VulkanBuffer b = createBuffer(accessor.getSize(), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		buffer = b.getBuffer();
	...
	@Override
	public void dispose() {
		vkUnmapMemory(Graphics.getVkContext().getDevice(), bufferMemory);
