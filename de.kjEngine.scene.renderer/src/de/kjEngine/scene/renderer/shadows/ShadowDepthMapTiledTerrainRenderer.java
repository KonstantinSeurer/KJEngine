 ...
 *
 */
public class ShadowDepthMapTiledTerrainRenderer extends Stage {

	private static final PipelineSource PIPELINE_SOURCE;
	static {
		PipelineSource source = null;
		try {
			source = PipelineSource.parse(RL.create("jar://scene.renderer/de/kjEngine/scene/renderer/shadows/depthMapTiledTerrain.shader"));
		...
		source.addDescriptorSet(TiledTerrainComponent.DESCRIPTOR_SET_SOURCE);
		source.addDescriptorSet(PbrMaterial.DESCRIPTOR_SET_SOURCE);
	 ...
	 * @param pipeline
	 */
	public ShadowDepthMapTiledTerrainRenderer(InputProvider inputProvider) {
		super(inputProvider);
